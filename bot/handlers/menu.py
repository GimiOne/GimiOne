...
async def buy(cb: CallbackQuery, db: Database, payments: PaymentService, cfg: Config, xui: XUIClient) -> None:
    ...
    # если это админ — подписка создаётся автоматически
    await ensure_admin_subscription(tg_id=cb.from_user.id, db=db, xui=xui, cfg=cfg)
...
async def my_sub(cb: CallbackQuery, db: Database, cfg: Config, xui: XUIClient) -> None:
    if cb.from_user is None:
        return
    if cb.message is None:
        await cb.answer()
...
async def get_key(cb: CallbackQuery, db: Database, cfg: Config, xui: XUIClient) -> None:
    if cb.from_user is None:
        return
    if cb.message is None:
        await cb.answer()
